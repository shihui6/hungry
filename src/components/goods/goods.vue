<template>
    <div class="goods">
        <div class="menu_wrapper" ref="menuwrapper">
            <ul>
                <li v-for="(item,index) of goods" :key="index" class="menu_item" @click="selectMenu(index,$event)" :class="{'current':currentIndex == index}"> 
                    <span class="text">
                      <span v-show="item.type>0" class="icon" :class="matchImage[item.type]"></span>  {{item.name}}
                    </span>
                </li>
            </ul>
        </div>
...
export default {
    ...
    methods:{
        // 在pc端的时候原生点击也是会被监听到的，所以在pc端就会有俩触发俩次点击事件
        // 要想解决这个问题，就是要在pc端阻止就可以
        selectMenu(index,event){
            if(!event._constructed){  //event._constructed这个属性是在better-scroll；里面生成的
                                    //在pc段是没有这个属性的，会显示undefined
                return;
            }
            let foodList = this.$refs.foodwrapper.getElementsByClassName('food-list-hook')
            console.log(foodList)
            let el = foodList[index]
            this.foodsScroll.scrollToElement(el,300)   //调取better-scroll接口，跳转到对应元素上
            console.log(index)
        },
        initScroll(){
            this.menuScroll = new BScroll(this.$refs.menuwrapper,{
                click:true  //开发了一个点击事件
            })
            this.foodsScroll = new BScroll(this.$refs.foodwrapper,{
                click:true ,
                // 可以实时监测到滚动的位置 相当于探针的效果
                probeType:3
            })
